...
def on_boundary(i, lines):
    for line in lines:
        ...
        if i in [line.i1 - 1, line.i2 - 1] and line.tag in [17, 18, 19, 20, 21, 22]:
            return True
    return False
...
def write_vtk(points, lines, triangles, filename, values):
    ...
    with open(filename, "w") as file:
        ...
        for line in lines:
            file.write(
                f"2 {line.i1 - 1} {line.i2 - 1}\n"
            )  # Adjusting to zero-based index

        # Writing triangles as polygons
        total_triangle_indices = sum(
            [3 for triangle in triangles]
        )  # 3 points per triangle
        file.write(
            f"POLYGONS {len(triangles)} {total_triangle_indices + len(triangles)}\n"
        )
        for triangle in triangles:
            file.write(
                f"3 {triangle.i1 - 1} {triangle.i2 - 1} {triangle.i3 - 1}\n"
            )  # Adjusting to zero-based index
